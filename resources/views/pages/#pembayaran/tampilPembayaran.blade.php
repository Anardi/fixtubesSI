        ...
        <div class="container">
            ...
            <div class="row justify-content-center">
                <div class="col-8">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('updatePembayaran', $pembayaran->id_pembayaran) }}" method="POST">
                                @csrf
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="id_pembayaran" class="form-control" id="exampleInputEmail1"
                                        aria-describedby="emailHelp" value="{{ $pembayaran->id_pembayaran }}">
                                </div>
                                <div class="mb-3">
                                    <label for="exampleInputEmail1" class="form-label">Pasien</label>
                                    <select name="kode_pasien" class="form-control">
                                        @foreach ($pasien as $item)
                                            <option value="{{ $item->kode_pasien }}" {{ $pembayaran->kode_pasien == $item->kode_pasien ? 'selected' : '' }}>
                                                {{ $item->kode_pasien . ' - ' . $item->Nama_pasien }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <select name="kode_obat" class="form-control">
                                        @foreach ($obat as $item)
                                            <option value="{{ $item->kode_obat }}" {{ $pembayaran->kode_obat == $item->kode_obat ? 'selected' : '' }}>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="Jumlah_bayar" class="form-control" id="exampleInputEmail1"
                                        aria-describedby="emailHelp" value="{{ $pembayaran->Jumlah_bayar }}">
                                </div>
